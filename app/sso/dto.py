...
class TokenRequestDto(BaseModel):
    ...
    code: Optional[str] = Field(None, description="Authorization Code (grant_type=authorization_code일 때)")
    redirect_uri: Optional[str] = Field(None, description="리다이렉트 URI (검증용)")
    client_id: str = Field(..., description="Client ID")
    client_secret: Optional[str] = Field(None, description="Client Secret (Confidential 클라이언트만)")
    refresh_token: Optional[str] = Field(None, description="Refresh Token (grant_type=refresh_token일 때)")
